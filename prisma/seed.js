...
async function main() {
  ...
  // precoVenda vira o nível de venda "Dúzia" (grão-base=1, já que aqui a dúzia inteira é o
  // menor grão vendável desses produtos de exemplo — ver NivelVendaProduto).
  const produtosSeed = [
  ...
  for (const { precoVenda, ...p } of produtosSeed) {
    ...
    const nivelExistente = await prisma.nivelVendaProduto.findFirst({ where: { produtoId, ehBase: true } });
    if (!nivelExistente) {
      await prisma.nivelVendaProduto.create({
        data: { produtoId, nome: 'Dúzia', quantidadeGrao: 1, preco: precoVenda, ehBase: true, precoManual: true },
      });
    }
  }
